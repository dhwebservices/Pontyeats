...
export async function sendCustomerOrderConfirmation({ to, order, restaurant }) {
  ...
  const inner = `
    ...
    <div style="margin-top:24px;color:#525252;font-size:14px">
      <div><strong>${order.delivery_type === 'collection' ? 'Collection from' : 'Delivering to'}:</strong> ${escapeHtml(order.delivery_type === 'collection' ? (restaurant.address_line || 'restaurant') : (order.delivery_address || ''))}</div>
...
export async function sendRestaurantNewOrderAlert({ to, order, restaurant }) {
  ...
  const itemsHtml = items.map(orderRow).join('');
  const inner = `
    <div style="background:#fef3eb;border:1px solid #f9c79b;color:#9a3412;padding:8px 12px;border-radius:8px;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.08em;margin-bottom:16px;display:inline-block">New order • ${order.delivery_type}</div>
    ...
    <div style="border:1px solid #ece6dd;border-radius:10px;padding:20px;background:#faf6f1">
      <table width="100%" style="font-size:14px;border-collapse:collapse">${itemsHtml}
        ...
        <tr><td colspan="2" style="padding:4px 0;color:#525252">Net to you (after fees)</td><td style="padding:4px 0;text-align:right;color:#525252">£${Number(order.net_to_restaurant||0).toFixed(2)}</td></tr>
      </table>
    </div>
